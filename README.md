...
| :--- | :--- | :--- | :--- |
| 流血表現（血など） | 描き方を知っている | なし | 対策不要（テキストエンコーダーの制限解除のみで出力可能） |
| 武器・兵器表現（銃など） | 描き方を知っている | なし | 対策不要（同上） |
| 性的表現（NSFWなど） | 描き方を知らない | なし | 追加学習が必要（データセットを用いたNSFW LoRAなどの作成） |
| 人体欠損（グロテスク表現） | 描き方を知らない | なし | 追加学習が必要（データセットを用いたGore LoRAなどの作成） |
...
*(※検証にあたっては、条件を揃えるために制限を解除したテキストエンコーダーを使用し、純粋なDiT側の挙動のみを測定しています)*
